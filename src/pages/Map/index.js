...
class Map extends Component{
    ...
    tipoVeiculoSelecionado = () => {
        let tipos = [this.state.carroComum,this.state.carroUtilitario,this.state.moto];
        let selecao = tipos.reduce(function(a,b){
            return Math.max(a,b);
        })
        return selecao;
    } 
   ...
   handleLocationSelected = (data,{geometry}) => {
    ...
    const {region,destination,confirmado,carroComum,carroUtilitario,moto} = this.state;
    ...
    return(
        <View style={{flex:1}}>
                ...
                { destination ?
                        <Fragment>
                        <TipoVeiculo 
                            selecionarCarroComum={this.carroComum} 
                            selecionarCarroUtilitario={this.carroUtilitario} 
                            selecionarMoto={this.moto}
                            back={this.back}
                            foto={ this.props.route.params?.fotoSalva}
                            deletarFoto={this.props.route.params?.deletar}
                        />
                        {(carroComum != 0 || carroUtilitario != 0 || moto != 0) &&
                            <Fragment>
                                <Directions
                                    origin={region}
                                    destination={destination}
                            ...
                            <Fragment>
                                ...
                                <AguardPedido 
                                    visivel = {true}
                                    back = {this.back}
                                    selecaoVeiculo = {this.tipoVeiculoSelecionado()}
                                />
                            </Fragment>
                        }
                        </Fragment> :
